...
def train(train_loader, model, optimizer, \
    ...
    for batch in pbar:
        ...
        der_x, der_y = utils.cheb_loss2(pred - gt, pred.size(-1), pred.size(-2), x_power = model.s\
                                                                                , y_power = model.s)
        loss = loss_fn(pred, gt) + loss_config['alpha']*(der_x.pow(2).mean() + der_y.pow(2).mean())
        ...
        for name, param in model.named_parameters():
            ...
            if last_name in ['s']:
                param_dict[name].append(param.data.item())

        
        torch.save(param_dict, save_path+'/model_parameters.pt')
...
def main(config_, save_path):
    ...
    for epoch in range(epoch_start, epoch_max + 1):
        ...
        sv_file = {
            ...
            'epoch': epoch,
            's' : model.state_dict()['s'].item()
        }

        torch.save(sv_file, os.path.join(save_path+'_epoch-last.pth'))

        # if prev_min_s > model.state_dict()['s'].item():
        #     prev_min_s = model.state_dict()['s'].item()
        #     torch.save(sv_file, os.path.join(save_path+'_min_s.pth'))

        # if prev_max_s < model.state_dict()['s'].item():
        #     prev_max_s = model.state_dict()['s'].item()
        #     torch.save(sv_file, os.path.join(save_path+'_max_s.pth'))
        ...
        if (epoch == 1) or ((epoch_val is not None) and (epoch % epoch_val == 0)) or (epoch==epoch_max):
...
#             writer.add_scalars('psnr', {'val': val_res}, epoch)
            if val_res > max_val_v:
                max_val_v = val_res
                torch.save(sv_file, os.path.join(save_path, config['save_name']+'_epoch-best.pth'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config',default='./configs/train_edsr-SoFoNO.yaml')
    ...
    alpha_value = config['loss']['alpha']
    save_path = os.path.join(config['save_dir'], save_name)

    ### Save Model Paramteter
    param_dict = {}
